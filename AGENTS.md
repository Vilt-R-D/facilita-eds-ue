...
The repository provides the basic structure, blocks, and configuration needed to run a complete site with `*.aem.live`.

### About aem-boilerplate-xwalk
A specific version of the aem-boilerplate project that provides a starting point for building websites with AEM EDS using AEM as a Cloud Service as the backend (as opposed as Google Drive or Microsoft Sharepoint). It also enables the use of AEM Universal Editor to author content.
